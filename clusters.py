...
# Extract documents and sentiment scores
documents = []
sentiment_scores = []
for entry in data:
    documents.append(entry['page_text'])
    sentiment_scores.append(entry.get('sentiment', 0))  # Assuming 'sentiment' is the key for sentiment score
...
average_cluster_sentiments = {cluster: np.mean(scores) if scores else 0 for cluster, scores in cluster_sentiments.items()}
